...
2. **Simulate Frontend Flow**:
   ...
   **Step A: Create 3 Computers**
   ```bash
   # Seed with CORRECT Inventory Data (Matches trained YOLO model)
   curl -X POST -H "Content-Type: application/json" -d '{"brand": "Lenovo", "code": "SKU-LAPTOP-let01", "price": 1200.00, "description": "Lenovo ThinkPad X1 Carbon"}' http://localhost:9000/api/computers
   # Note: al01 is no longer in the best.pt model, but we can keep it as legacy
   # curl -X POST -H "Content-Type: application/json" -d '{"brand": "HP", "code": "SKU-LAPTOP-al01", "price": 800.00, "description": "Laptop HP económica"}' http://localhost:9000/api/computers
   
   curl -X POST -H "Content-Type: application/json" -d '{"brand": "Asus", "code": "SKU-LAPTOP-asu01", "price": 1500.00, "description": "Asus ROG Strix Gaming"}' http://localhost:9000/api/computers
   curl -X POST -H "Content-Type: application/json" -d '{"brand": "Apple", "code": "SKU-LAPTOP-mbk01", "price": 2000.00, "description": "MacBook Pro M1 14ulg"}' http://localhost:9000/api/computers
   ```
